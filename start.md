---
layout: layout.njk
title: Start here
permalink: /start/index.html
---

# Start here

This is the build timeline for our Ford Transit van, from the first clean-out through the current interior build.

## Phase 1: basic camper

- [Beginning](/posts/Beginning/) - buying the van and starting the build
- [Van Clean and Reupholstery](/posts/VanClean/) - cleaning out the used cargo van and repairing the front seats
- [Swivel Seats](/posts/SwivelSeats/) - making the front seats part of the living space
- [Sound Deadening](/posts/SoundDeadening/) - treating rust and quieting the van
- [Installing Bench Seat/Folding Bed](/posts/BenchSeat/) - adding the Scopema bench seat and first bed
- [Floor and Kitchen](/posts/FloorAndKitchen/) - replacing the floor and adding the first kitchen setup
- [Bed Install](/posts/BedInstall/) - adding the adult bed
- [Fan Install](/posts/FanInstall/) - cutting the roof and installing ventilation
- [Final Essentials first Van Phase](/posts/FinalEssentials/) - bug nets, water, lights, and trip-ready basics
- [Summer 2025 Camping](/posts/Summer2025/) - first season using the van

## Phase 2: making it work better

- [Phase 2 Van Build](/posts/Phase2Build/) - what we learned from summer and what needed to change
- [Bench Build](/posts/BenchBuild/) - building the garage workbench to support the project
- [Custom Roof Rack](/posts/RoofRack/) - building a rack for solar
- [Solar](/posts/Solar/) - adding 350W of solar
- [DC to DC Charger](/posts/DCAlternator/) - charging from the alternator
- [Electrical](/posts/Electrical/) - planning and running the main wiring
- [Installing Van Capsules](/posts/Capsules/) - widening the rear sleeping area
- [Bed and Wall Frame](/posts/BedAndWallFrame/) - adding wall cleating and raising the bed
- [Insulation](/posts/Insulation/) - adding Havelock Wool
- [Electrical Part 2](/posts/ElectricalPart2/) - installing useful lights and USB power
- [Headliner, Pockets, and Fridge Cabinet](/posts/HeadlinerPocketsFridgeCabinet/) - adding storage and a secure fridge cabinet
- [Wire Harness](/posts/WireHarness/) - relocating the factory wiring before the ceiling and walls went in
- [Ceiling Framing](/posts/CeilingFraming/) - building the structure for the plywood ceiling
- [New Radio](/posts/NewRadio/) - replacing the radio and adding a four-camera system
- [Installing the Ceiling](/posts/Ceiling/) - adding L-track, Baltic birch panels, lights, trim, and fan covers
- [Upper Walls and Cabinets](/posts/UpperWallsAndCabinets/) - finishing the upper walls and adding organized storage
- [Van Capsule Interior](/posts/VanCapsuleInterior/) - trimming out the window capsules with custom boxes, panels, and upholstery

## Current direction

The ceiling, upper walls, lighting, upper cabinets, and capsule window trim are now installed. That work transformed the van from a collection of exposed systems and framing into a much more finished and usable interior.

We are still using the van between projects, which lets us test each addition before deciding what to build next. The current focus is on the permanent kitchen, a fold-down table, and final kitchen electrical.

## What's next

For the rest of this summer, the plan is to keep using the van while we keep building it. We expect to do one or two weekend trips a month, keep testing what works, and continue planning the rest of Phase 2.

The next build priorities are:

- Remaining wall work and a fold-down table
- A permanent wood kitchen designed for the van, with more drawers, cabinets, and a nook for spices
- Final electrical, including kitchen lights and a kitchen GFCI plug

We imagine this work will happen intermittently through the summer and into the fall. As always, we are trying to take our time, use the van between projects, and let that experience guide the final design.

Tentatively, we are thinking about taking our first real one-week trips next spring, likely within a few hundred miles of Sacramento. Those trips will be the first bigger test of the water systems, electrical system, refrigerator, and how well our supplies work for a longer stretch.

After that, once we can make a two-week trip work, the first big target on the dream list is the Southwest, including the Grand Canyon. There are plenty of other ideas after that, but for now we are still pretty focused on finishing the build.
